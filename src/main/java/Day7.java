...
public class Day7 {
    ...
    static String solve(final String input) {
        final List<TowerElement> towerElements = Arrays.stream(input.split("\n")).map(Day7::extractObject)
                .collect(Collectors.toList());

        // remove all children until there is only 1 element left
        while (towerElements.size() > 1) {
            towerElements.removeIf(el -> !hasChildren(el, towerElements));
        }

        return towerElements.get(0).getName();
    }

    private static boolean hasChildren(final TowerElement element, final List<TowerElement> towerElements) {
        if (element.getChildren() == null) {
            return false;
        }
        for (final TowerElement towerElement : towerElements) {
            for (final String childName : element.getChildren()) {
                if (towerElement.getName().equals(childName)) {
                    return true;
                }
            }
        }
    ...
    static TowerElement extractObject(String line) {
        String[] children = null;
        if (line.contains(" -> ")) {
            final String[] split = line.split(" -> ");
            children = split[1].replace(" ", "").split(",");
            line = split[0];
        }
        final String[] split = line.split(" ");
        final String name = split[0];
        final Integer weight = Integer.parseInt(split[1].replace("(", "").replace(")", ""));
        return new TowerElement(name, weight, children);
    }

    public static final String PUZZLE = "dihjv (2158) -> gausx, ncdmp, hozgrub\n"
                                        ...
                                        + "qujsvg (58)\n"
                                        + "bdnqwvw (10384) -> mqyvpvw, kfaib, rpgxh, kjudskk, gknpczm, mtukvee, "
                                        + "xwbzev\n"
                                        + "lfjwzpr (80)\n"
                                        + "rpdiks (56)\n"
                                        + "kveyw (87)\n"
                                        ...
                                        + "iwvgsqj (25)\n"
                                        + "yghqsa (56478) -> vakhjoj, klyfish, rbgidcj, zsksm, mgdjgzf, gnrgy, "
                                        + "zlwnppb\n"
                                        + "nedixbk (6)\n"
                                        + "kbseli (63)\n"
                                        + "zppzac (71)\n"
